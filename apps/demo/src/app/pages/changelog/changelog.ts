...
@Component({
  ...
  standalone: true,
  imports: [NgxsmkCard, NgxsmkCardContent, NgxsmkCardHeader, NgxsmkCardTitle, NgxsmkBadge, NgxsmkHeading, NgxsmkText, AppNav],
  template: `
    <app-nav />
    <div class="ngxsmk-page">
      <header class="ngxsmk-page__header">
        <ngxsmk-heading level="h1">Changelog</ngxsmk-heading>
        <ngxsmk-text variant="body" class="ngxsmk-page__sub">Keep track of the changes, bug fixes, updates, and releases of NGXSMK.</ngxsmk-text>
      </header>
        ...
        @for (release of releases; track release.version) {
          <ngxsmk-card class="ngxsmk-release">
            ...
            <div ngxsmkCardContent>
              <ngxsmk-text variant="body" class="ngxsmk-changelog-desc">{{ release.summary }}</ngxsmk-text>
              <ul class="ngxsmk-release__changes">
                @for (change of release.changes; track change.text) {
                  <li>
                    <ngxsmk-badge
                      [variant]="change.type === 'added' ? 'success' : change.type === 'fixed' ? 'warning' : 'primary'"
  ...
  `,
  styles: `
    .ngxsmk-page { max-width: 980px; margin: 0 auto; padding: var(--ngxsmk-space-12,3rem) var(--ngxsmk-space-6,1.5rem); }
    ...
    .ngxsmk-changelog-list { display: flex; flex-direction: column; gap: var(--ngxsmk-space-5,1.25rem); }
    .ngxsmk-release__top { display: flex; align-items: center; justify-content: space-between; gap: var(--ngxsmk-space-2,0.5rem); }
    .ngxsmk-changelog-desc { color: var(--ngxsmk-color-on-surface-variant,#71717a); font-size: 0.875rem; line-height: 1.6; margin: 0 0 var(--ngxsmk-space-3,0.75rem); }
    .ngxsmk-release__changes { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: var(--ngxsmk-space-2,0.5rem); }
    .ngxsmk-release__changes li { display: flex; align-items: center; gap: var(--ngxsmk-space-2,0.5rem); font-size: 0.875rem; color: var(--ngxsmk-color-on-surface,#09090b); }
  `,
})
export class ChangelogPage {
  protected readonly releases: Release[] = [
    ...
    },
    {
      ...
      changes: [
        { type: 'added', text: 'Foundational primitives: button, card, input, dialog.' },
      ],
    },
  ];
}
