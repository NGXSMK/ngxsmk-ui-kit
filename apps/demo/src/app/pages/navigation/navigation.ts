import { NgxsmkSideNavHeading, NgxsmkSideNavItem, NgxsmkSideNavSection, NgxsmkSideNav, NgxsmkSideNavCollapseButton } from '@ngxsmk/core/side-nav';
import { NgxsmkTopNavHeading, NgxsmkTopNavItem, NgxsmkTopNavMenu, NgxsmkTopNavMegaMenu, NgxsmkTopNavMegaMenuItem, NgxsmkTopNav, NgxsmkTopNavMegaMenuFeaturedCard } from '@ngxsmk/core/top-nav';
...
@Component({
  ...
  imports: [
    ShowcaseExample,
    NgxsmkBreadcrumbItem, NgxsmkOutline, NgxsmkTabMenu, NgxsmkNavIcon, NgxsmkNavHeadingMenu, NgxsmkLinkProvider,
    NgxsmkSideNav, NgxsmkSideNavHeading, NgxsmkSideNavItem, NgxsmkSideNavSection, NgxsmkSideNavCollapseButton,
    NgxsmkTopNav, NgxsmkTopNavHeading, NgxsmkTopNavItem, NgxsmkTopNavMenu,
    NgxsmkTopNavMegaMenu, NgxsmkTopNavMegaMenuItem, NgxsmkTopNavMegaMenuFeaturedCard,
    NgxsmkMobileNav, NgxsmkMobileNavToggle,
  ],
  template: `
    <h2 class="ngxsmk-page-title">Navigation</h2>
    <p class="ngxsmk-page-desc">
      Navigation primitives — from tiny breadcrumbs to full app shells — that
      share the theme token engine, so every link, tab, and menu recolors with
      your brand.
    </p>
    ...
    >
      <ngxsmk-tab-menu>
        ...
        <button class="ngxsmk-tab-menu__tab" role="tab" [class.ngxsmk-tab-menu__tab--active]="tabActive() === 'settings'" (click)="tabActive.set('settings')">Settings</button>
      </ngxsmk-tab-menu>
    </showcase-example>
    ...
    >
      <span class="ngxsmk-sc-wrap">
        ...
        <ngxsmk-nav-icon label="Search" [active]="navIconActive() === 'search'" (click)="navIconActive.set('search')">
          <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4-4"/></svg>
        </ngxsmk-nav-icon>
        <ngxsmk-nav-icon label="Settings" size="lg" [active]="navIconActive() === 'settings'" (click)="navIconActive.set('settings')">
          <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19 12a7 7 0 0 0-.1-1l2-1.5-2-3.4-2.3 1a7 7 0 0 0-1.7-1l-.3-2.5h-4l-.3 2.5a7 7 0 0 0-1.7 1l-2.3-1-2 3.4 2 1.5a7 7 0 0 0 0 2l-2 1.5 2 3.4 2.3-1a7 7 0 0 0 1.7 1l.3 2.5h4l.3-2.5a7 7 0 0 0 1.7-1l2.3 1 2-3.4-2-1.5a7 7 0 0 0 .1-1z"/></svg>
        </ngxsmk-nav-icon>
      </span>
    </showcase-example>
      ...
      <div style="height: 320px; position: relative; border: 1px solid var(--ngxsmk-color-outline); border-radius: var(--ngxsmk-radius-lg); overflow: hidden;">
        <ngxsmk-side-nav>
          <ngxsmk-side-nav-section>
            ...
            <ngxsmk-side-nav-item [active]="sideNavActive() === 'dashboard'" (clicked)="sideNavActive.set('dashboard')">Dashboard</ngxsmk-side-nav-item>
            <ngxsmk-side-nav-item [active]="sideNavActive() === 'analytics'" (clicked)="sideNavActive.set('analytics')" badge="3">Analytics</ngxsmk-side-nav-item>
            <ngxsmk-side-nav-item [active]="sideNavActive() === 'reports'" (clicked)="sideNavActive.set('reports')">Reports</ngxsmk-side-nav-item>
          </ngxsmk-side-nav-section>
          <ngxsmk-side-nav-section>
            <ngxsmk-side-nav-heading>Account</ngxsmk-side-nav-heading>
            <ngxsmk-side-nav-item [active]="sideNavActive() === 'profile'" (clicked)="sideNavActive.set('profile')">Profile</ngxsmk-side-nav-item>
            <ngxsmk-side-nav-item [active]="sideNavActive() === 'billing'" (clicked)="sideNavActive.set('billing')">Billing</ngxsmk-side-nav-item>
          </ngxsmk-side-nav-section>
    ...
    >
      <div style="height: 200px; position: relative; width: 100%; overflow: hidden; border: 1px solid var(--ngxsmk-color-outline); border-radius: var(--ngxsmk-radius-lg);">
        <ngxsmk-top-nav>
          ...
          <ngxsmk-top-nav-item [active]="topNavActive() === 'home'" (clicked)="topNavActive.set('home')">Home</ngxsmk-top-nav-item>
          <ngxsmk-top-nav-item [active]="topNavActive() === 'about'" (clicked)="topNavActive.set('about')">About</ngxsmk-top-nav-item>
          <ngxsmk-top-nav-menu>
      ...
      <div style="height: 280px; position: relative; width: 100%; overflow: hidden; border: 1px solid var(--ngxsmk-color-outline); border-radius: var(--ngxsmk-radius-lg);">
        <ngxsmk-top-nav>
          ...
          <ngxsmk-top-nav-mega-menu [featured]="true">
            Products ▾
            <div mega-column>
              <ngxsmk-top-nav-mega-menu-item title="Analytics" description="Track your metrics" href="#" />
              <ngxsmk-top-nav-mega-menu-item title="Automation" description="Build workflows" href="#" />
              <ngxsmk-top-nav-mega-menu-item title="Insights" description="Surface trends" href="#" />
            </div>
            <div mega-column>
              <ngxsmk-top-nav-mega-menu-item title="Reports" description="Share progress" href="#" />
              <ngxsmk-top-nav-mega-menu-item title="Integrations" description="Connect tools" href="#" />
    ...
    >
      <div class="ngxsmk-sc-surface" style="position: relative; overflow: hidden;">
        ...
        <ngxsmk-mobile-nav [open]="mobileNavOpen()" (openedChange)="mobileNavOpen.set($event)" label="Acme">
        ...
        <ngxsmk-side-nav>
          <ngxsmk-side-nav-section>
            ...
            @for (item of sideNavComposedMain; track item.id) {
              <ngxsmk-side-nav-item [active]="sideNavComposedActive() === item.id" (clicked)="sideNavComposedActive.set(item.id)">{{ item.label }}</ngxsmk-side-nav-item>
            }
          </ngxsmk-side-nav-section>
          <ngxsmk-side-nav-section>
            <ngxsmk-side-nav-heading>Account</ngxsmk-side-nav-heading>
      ...
      <div style="height: 220px; position: relative; width: 100%; overflow: hidden; border: 1px solid var(--ngxsmk-color-outline); border-radius: var(--ngxsmk-radius-lg);">
        <div class="ngxsmk-top-nav">
          ...
          @for (item of topNavComposedLinks; track item.id) {
            <ngxsmk-top-nav-item [active]="topNavComposedActive() === item.id" (clicked)="topNavComposedActive.set(item.id)">{{ item.label }}</ngxsmk-top-nav-item>
          }
          <ngxsmk-top-nav-menu>
            <ngxsmk-top-nav-item>Resources ▾</ngxsmk-top-nav-item>
          </ngxsmk-top-nav-menu>
        ...
        <div class="ngxsmk-top-nav-composed__mega">
          @for (item of topNavComposedMega; track item.title) {
            <ngxsmk-top-nav-mega-menu-item [title]="item.title" [description]="item.description" href="#" />
          }
  ...
  styles: `
    :host { display: block; }
    .ngxsmk-demo-hint { margin: 0.5rem 0 0; font-size: 0.8125rem; color: var(--ngxsmk-color-on-surface-variant); }
    ...
    .ngxsmk-demo-mobile-link:hover { background: var(--ngxsmk-color-surface-hover); }
    ...
    .ngxsmk-link:hover { background: var(--ngxsmk-color-surface-hover); }
    .ngxsmk-link--active { color: var(--ngxsmk-color-primary); background: var(--ngxsmk-color-primary-container); }
