...
@Component({
  ...
  template: `
    ...
    <div class="ngxsmk-page">
      <header class="ngxsmk-page__header">
        ...
        <ngxsmk-text variant="body" class="ngxsmk-page__sub">A preview of what we're working on and what's coming next to NGXSMK.</ngxsmk-text>
      </header>
          ...
          <ngxsmk-card class="ngxsmk-milestone">
            <div ngxsmkCardHeader>
              <div class="ngxsmk-milestone__top">
                <ngxsmk-heading level="h3" ngxsmkCardTitle>{{ m.quarter }} — {{ m.title }}</ngxsmk-heading>
                <ngxsmk-badge
                  [variant]="m.status === 'shipped' ? 'success' : m.status === 'in-progress' ? 'warning' : 'primary'"
                >{{ m.status === 'shipped' ? 'Shipped' : m.status === 'in-progress' ? 'In progress' : 'Planned' }}</ngxsmk-badge>
              </div>
            </div>
  ...
  styles: `
    ...
    .ngxsmk-roadmap-list { display: flex; flex-direction: column; gap: var(--ngxsmk-space-5,1.25rem); }
    .ngxsmk-milestone__top { display: flex; align-items: center; justify-content: space-between; gap: var(--ngxsmk-space-2,0.5rem); }
    .ngxsmk-milestone__items { margin: 0; padding-left: 1.25rem; color: var(--ngxsmk-color-on-surface,#09090b); font-size: 0.875rem; line-height: 1.8; }
  `,
})
