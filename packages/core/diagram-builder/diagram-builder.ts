...
@Component({
  ...
  template: `
    <svg class="ngxsmk-diagram-builder__svg">
      @for (edge of edges(); track $index) {
        <line
          class="ngxsmk-diagram-builder__edge"
          [attr.x1]="nodeX(edge.from)"
          [attr.y1]="nodeY(edge.from)"
          [attr.x2]="nodeX(edge.to)"
          [attr.y2]="nodeY(edge.to)"
        />
      }
      @for (node of nodes(); track node.id) {
        <g class="ngxsmk-diagram-builder__node" (click)="selected.emit(node.id)">
          ...
          <text [attr.x]="node.x + 60" [attr.y]="node.y + 24" text-anchor="middle">
            {{ node.label }}
          </text>
        </g>
      }
  ...
  styles: `
    ...
    .ngxsmk-diagram-builder__svg {
      width: 100%;
      height: 100%;
      min-height: 20rem;
    }
    .ngxsmk-diagram-builder__edge {
      stroke: var(--ngxsmk-color-outline);
      stroke-width: 2;
    }
    .ngxsmk-diagram-builder__node rect {
      fill: var(--ngxsmk-color-surface);
      stroke: var(--ngxsmk-color-outline);
      stroke-width: 1;
      cursor: pointer;
    }
    .ngxsmk-diagram-builder__node:hover rect {
