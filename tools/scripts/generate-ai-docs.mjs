...
// The demo renders APP_VERSION as the current stable release. Stamping it here
// keeps the deployed site from advertising a version the packages no longer
// are — the same reason the manifests above are generated rather than edited.
const demoVersionFile = join(root, 'apps', 'demo', 'src', 'app', 'core', 'version.ts');
if (existsSync(demoVersionFile)) {
  writeFileSync(
    demoVersionFile,
